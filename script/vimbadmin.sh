...
if [[ ${USE_PHP71} = '1' ]]; then
	sed -i 's/fastcgi_pass unix:\/var\/run\/php\/php7.0-fpm.sock\;/fastcgi_pass unix:\/var\/run\/php\/php7.1-fpm.sock\;/g' /etc/nginx/sites-custom/vimbadmin.conf
fi
...
if [[ ${USE_PHP71} = '1' ]]; then
		systemctl restart {dovecot,postfix,amavis,spamassassin,clamav-daemon,nginx,php7.1-fpm,mysql} >>"$main_log" 2>>"$err_log" || error_exit "Cannot restart services! Aborting"
fi
